...
#' @examples ## sample fasta of Biostrings package
#' # fp <- system.file("extdata", "AtMlos.fna", package="rskodat")
#' # bndb <- "~/db/cdna/TAIR10_cdna" # set you environment
#' # bnout <- rblast(in_f = fp, out_f = "-", program = "blastn", db = bndb, "-num_threads 4")
#' # clmns <- c("qseqid", "sseqid", "pident", "qstart", "qend", "sstart",
...
#'
#' @importFrom dplyr %>% group_by mutate mutate_at vars funs ungroup arrange filter row_number
#' @importFrom RColorBrewer brewer.pal
#' @importFrom ggplot2 ggplot theme_bw geom_segment scale_color_manual facet_wrap theme element_text element_blank scale_y_discrete position_nudge
...
ggblast <- function(blast_out, drawtype){
  ...
  # blast_out[which(names(blast_out) %in% num_col)] <-
  #   data.frame(lapply(blast_out[which(names(blast_out) %in% num_col)], as.numeric),
  #              stringsAsFactors = F)
  ...
  # insert query line to dataframe
  ## insert position ----
  ...
  qseqid = NULL; sseqid = NULL; pident = NULL; sstrand = NULL; ord = NULL; qsid = NULL; qstart = NULL; qend = NULL
  ggdat <- qsdat %>%
    dplyr::group_by(qseqid, sseqid) %>%
    dplyr::mutate(rank = dplyr::row_number(sseqid)) %>%
    dplyr::ungroup(qseqid, sseqid) %>%
    dplyr::mutate(pident=as.numeric(as.character(cut(pident, breaks=c(0,seq(70,100,5)), 8:2)))) %>%
    ...
    dplyr::mutate(pident=factor(pident),
                  sseqid=factor(sseqid, levels = rev(unique(qsdat$sseqid))),
  ...
  x <- c(0,seq(70,100,5))
  idrange <- rev(sapply(head(seq_along(x), -1), function(i) paste(x[i], x[i+1], sep=":")))
